...
def gen_subgraph_datasets(args):
    ...
    train_g.ndata['global_id'] = torch.arange(train_g.num_nodes())
    entity_attrs = data['train_graph'].get('entity_attrs', {})

    BYTES_PER_DATUM = get_average_subgraph_size(args, args.num_sample_for_estimate_size, train_g, entity_attrs) * 2
    map_size = max((args.num_train_subgraph + args.num_valid_subgraph) * BYTES_PER_DATUM,
                   1024 * 1024 * 64)
    ...
    for idx in tqdm(range(args.num_train_subgraph)):
        ...
        datum = sample_one_subgraph(args, train_g, entity_attrs)
...
def sample_one_subgraph(args, bg_train_g, entity_attrs=None):
    ...
    while True:
        while True:
            ...
            min_nodes = min(getattr(args, 'min_subgraph_nodes', 50), bg_train_g.num_nodes())
            if sub_g.num_nodes() >= min_nodes:
                break
    ...
    local_entity_attrs = {}
    if entity_attrs is not None and 'global_id' in sub_g.ndata:
        for sub_node_id, local_node_id in ent_reidx.items():
            global_node_id = int(sub_g.ndata['global_id'][sub_node_id].item())
            attrs = entity_attrs.get(global_node_id, [])
            if attrs:
                local_entity_attrs[local_node_id] = attrs

    # hr2t, rt2h
    hr2t, rt2h = get_hr2t_rt2h_sup_que(sup_tris, que_tris)

    return sup_tris, que_tris, local_entity_attrs, hr2t, rt2h


def get_average_subgraph_size(args, sample_size, bg_train_g, entity_attrs=None):
    total_size = 0
    for i in range(sample_size):
        datum = sample_one_subgraph(args, bg_train_g, entity_attrs)
