...
class RGCNLayer(nn.Module):
    ...
    def msg_func(self, edges):
        # 检查edges.data['type']的最大值和最小值是否在合法范围内
        # max_type_index = edges.data['type'].max().item()
        # min_type_index = edges.data['type'].min().item()
        # print("Max type index:", max_type_index, "Min type index:", min_type_index, "Rel weight size:", self.rel_weight.size(0))

        # 断言确保类型索引在合法范围内
        # assert max_type_index < self.rel_weight.size(0) and min_type_index >= 0, "Edge type index out of bounds"

        # w = self.rel_weight.index_select(0, edges.data['type'])
        safe_indices = torch.clamp(edges.data['type'], 0, self.rel_weight.size(0) - 1)
        w = self.rel_weight.index_select(0, safe_indices)
