...
def add_node_attrs(g, entity_attrs, num_attr):
    if not entity_attrs:
        return

    padding_idx = num_attr
    node_attrs = []
    max_attr_len = 1
    for node_id in range(g.num_nodes()):
        attrs = list(entity_attrs.get(node_id, []))
        node_attrs.append(attrs)
        max_attr_len = max(max_attr_len, len(attrs))

    attr_ids = torch.full((g.num_nodes(), max_attr_len), padding_idx, dtype=torch.long)
    attr_mask = torch.zeros((g.num_nodes(), max_attr_len), dtype=torch.float32)
    for node_id, attrs in enumerate(node_attrs):
        if not attrs:
            continue
        attrs = attrs[:max_attr_len]
        attr_ids[node_id, :len(attrs)] = torch.tensor(attrs, dtype=torch.long)
        attr_mask[node_id, :len(attrs)] = 1.0

    g.ndata['attr_ids'] = attr_ids
    g.ndata['attr_mask'] = attr_mask


def get_g_bidir(triples, args, entity_attrs=None):
    ...
    add_node_attrs(g, entity_attrs, args.num_attr)
    return g
...
def get_indtest_test_dataset_and_train_g(args):
    ...
    g = get_g_bidir(torch.LongTensor(data['train']), args, data.get('entity_attrs'))
...
def get_num_attr(args):
    data = pickle.load(open(args.data_path, 'rb'))
    return int(data.get('num_attr', 0))
