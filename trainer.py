...
from new_ent_init_model import EntInit
...
class EntityClassifier(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        self.fc = nn.Linear(input_dim, num_classes)  # 创建一个全连接层

    def forward(self, x):
        x = self.fc(x)  # 应用线性变换
        return F.log_softmax(x, dim=1)  # 返回log-softmax结果，适合与NLLLoss结合使用
